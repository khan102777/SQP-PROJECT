...
select CNAME FROM (select CLIENT.NAME AS CNAME, PACKAGE.WEIGHT AS WEI 
                    FROM CLIENT JOIN PACKAGE ON CLIENT.ACCOUNTNUMBER = PACKAGE.RECIPIENT) AS A
where WEIGHT = 1.5;
    
-- 7.2 What is the total weight of all the packages that he sent?
select sum(WEI) from (select CLIENT.NAME AS CNAME, PACKAGE.WEIGHT AS WEI 
                      FROM CLIENT JOIN PACKAGE ON CLIENT.ACCOUNTNUMBER = PACKAGE.RECIPIENT) AS A
WHERE CNAME = "Al Gore's Head";

select CLIENT.NAME AS CNAME, PACKAGE.WEIGHT AS WEI 
